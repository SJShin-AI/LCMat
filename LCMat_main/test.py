    ...
    if 'idc' in args.slct_type:
        ...
        if name == '':
            if args.dataset == 'cifar10':
                ...
                name = f'cifar100_mp/conv3in_train_{args.training_mode}_'
                if args.match == 'curvature_ftorch':
                    name+=f'var{args.var_weight}_on_False_ema_{args.ema}'
                elif args.match == 'feat':
                    name+='f2'
                else:
                    name+='grad_fc'
                name += f'net_{args.net_update}_{args.metric}_nd500_cut_niter{args.niter}_factor{args.factor}_lr0.005_{init}'
...
def load_data_path(args):
    ...
    if args.dataset == 'imagenet':
        ...
        # Load condensed dataset
        ###### modified by hs@221006
        if 'idc' in args.slct_type:
            if args.slct_type == 'idc':
                args.logger = Logger_test(args.save_dir)
                args.logger(f"Save dir: {args.save_dir}")

                data, target = torch.load(os.path.join(f'{args.save_dir}', 'data.pt'))
            else: # idc_cat
                args.logger = Logger_test(os.path.join(f'{args.save_dir}_{args.nclass_sub}_phase0','mp'))
                args.logger(f"Save dir: {args.save_dir}_{args.nclass_sub}_mp")

                data_all = []
                target_all = []
                for idx in range(args.nclass // args.nclass_sub):
                    path = f'{args.save_dir}_{args.nclass_sub}_phase{idx}'
                    data, target = torch.load(os.path.join(path, 'mp', 'data.pt'))
                    data_all.append(data)
                    target_all.append(target)
                    print(f"Load data from {path}")
...
def test_data(args,
    ...
    for model_fn in model_fn_ls:
        ...
        logger(
            f'Repeat {repeat} => Best, last acc standard deviation: {np.std(best_acc_l):.1f} {np.std(acc_l):.1f}\n')

        try:
            if test_resnet:
                pass
            else:
                ckpt_path = os.path.join(args.save_dir, f'checkpoint{args.it}_synthetic.pth.tar')
                torch.save(model.state_dict(), ckpt_path)
        except:
            pass
        try:
            if test_resnet:
                wandb.log({"resnet_best_acc": np.mean(best_acc_l),"resnet_acc": np.mean(acc_l)})
            else:
                wandb.log({"best_acc": np.mean(best_acc_l),"acc": np.mean(acc_l)})
        except:
            pass

if __name__ == '__main__':
    from misc.utils import Logger_test
    ...
    args.device = torch.device("cuda:" + str(args.gpu)) if torch.cuda.is_available() else torch.device("cpu")
    ...
    for p in path_list:
        ...
        test_data(args, train_loader, val_loader, repeat=args.repeat, test_resnet=False, logger=args.logger)
        if args.dataset[:5] == 'cifar':
            test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=resnet10_bn, logger=args.logger)
            if (not args.same_compute) and (args.ipc >= 50 and args.factor > 1):
                args.epochs = 400
            test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=densenet, logger=args.logger)
        elif args.dataset == 'imagenet':
            test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=resnet18_bn, logger=args.logger)
            test_data(args, train_loader, val_loader, repeat=args.repeat, model_fn=efficientnet, logger=args.logger)