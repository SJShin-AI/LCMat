...
class ResNet_32x32(nn.Module):
    def __init__(self, block, num_blocks, channel=3, num_classes=10, record_embedding: bool = False,
                 no_grad: bool = False,backpack = False):
        super().__init__()
        self.in_planes = 64
        ...
        if backpack:
            self.linear = extend(nn.Linear(512 * block.expansion, num_classes))
        else:
...
def ResNet(arch: str, channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
           pretrained: bool = False, backpack = False):
    arch = arch.lower()
    if pretrained:
        if arch == "resnet18":
        ...
        if arch == "resnet18":
            net = ResNet_32x32(BasicBlock, [2, 2, 2, 2], channel=channel, num_classes=num_classes,
                               record_embedding=record_embedding, no_grad=no_grad,backpack=backpack)
        elif arch == "resnet34":
...
def ResNet18(channel: int, num_classes: int, im_size, record_embedding: bool = False, no_grad: bool = False,
             pretrained: bool = False, backpack = backpack):
    return ResNet("resnet18", channel, num_classes, im_size, record_embedding, no_grad, pretrained,backpack)
